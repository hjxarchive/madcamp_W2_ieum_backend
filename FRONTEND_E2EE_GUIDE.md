# 프론트엔드 E2EE 구현 가이드 (커플 연결 시 암호화 키 생성)

## 🔐 보안 원칙
**⚠️ CRITICAL: RSA Private Key는 절대 서버로 전송하거나 공유하지 마세요!**

---

## 📋 전체 플로우

### 1단계: 로그인 후 RSA Key Pair 생성 (각 사용자)

```kotlin
// User A, User B 각각 실행
fun onGoogleLogin(idToken: String) {
    // 1. 백엔드에 Google OAuth 로그인
    val response = authApi.googleLogin(idToken)
    val jwt = response.accessToken
    val userId = response.user.id
    
    // 2. RSA Key Pair 생성 (로컬에서만)
    val keyPair = generateRSAKeyPair()
    val privateKey = keyPair.private  // 🔒 로컬 Secure Storage에 저장
    val publicKeyString = encodePublicKey(keyPair.public)
    
    // 3. Public Key만 서버로 전송
    userApi.registerPublicKey(jwt, publicKeyString)
    
    // 4. Private Key를 안전하게 저장
    secureStorage.savePrivateKey(userId, privateKey)
}
```

---

### 2단계: 커플 연결 시 AES Shared Key 생성 (User B가 실행)
...
User A                            Backend                      User B
  |                                  |                            |
1.로그인 → RSA Key Pair 생성          |                            |
  Private Key A (로컬 저장) 🔒        |                            |
  Public Key A → 서버 전송 ──────────►| Store Public Key A        |
  |                                  |                            |
  |                                  |                     2.로그인 → RSA Key Pair 생성
  |                                  |                       Private Key B (로컬 저장) 🔒
  |                                  |◄────────────────────  Public Key B 전송
  |                                  | Store Public Key B    |
  |                                  |                            |
3.초대 코드 생성 ────────────────────►| Store Invite Code         |
  |                                  |                            |
  | ─────────── 초대 코드 공유 (문자/카톡) ────────────────────►  |
  |                                  |                            |
  |                                  |◄────────────────────  4.커플 연결 요청
  |                                  | Return Partner's          ↓
  |                                  | Public Key A          5.AES Shared Key 생성
  |                                  |                            ↓
  |                                  |                       6.Public Key A로 암호화
  |                                  |                         → encryptedForA
  |                                  |                            ↓
  ...
  |                                  |                            |
9.앱 재실행 시                        |                            |
  암호화된 AES Key 요청 ──────────────►| Return encryptedForA      |
  ↓                                  |                            |
10.Private Key A로 복호화            |                            |
  → AES Shared Key ✓                 |                            |

┌─────────────────────────────────────────────────────────────┐
│  이후 모든 메시지 (공유 AES Shared Key 사용)                  │
└─────────────────────────────────────────────────────────────┘
